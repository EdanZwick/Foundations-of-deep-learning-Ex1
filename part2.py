...
class Linear:
    def __init__(self, D_in, D_out, std=0.02, xavier=False):
        if xavier:
            u = np.sqrt(6 / (D_in + D_out))
            self.w = torch.empty((D_in + 1, D_out), dtype=torch.float).uniform_(-std, std)
        else:
...
#  P is the probability for a neuron to be droped out.
#  drop out is done by multiplying with a binary mask.
#  train time results are devided by p so value's expectaition will be the same during test time (where no drop-out is done).
class Dropout:
    ...
    def forward(self, x):
        if self.train:
            ...
            x = x * mask / self.p  # all multipicaion and division is done element wise
        return x


class Net:
    def __init__(self, din, dout, dhidden, std=0.02, xavier=False, dropout_p=0, train=False):
        self.layers = []
        for dim in dhidden:
            self.layers.append(Linear(din, dim, std, xavier))
            din = dim
        self.output = Linear(din, dout, std)  # output  layer
        self.dropout_p = dropout_p

    def forward(self, x):
        self.dropout = Dropout(self.dropout_p, train) if (train and self.dropout_p > 0) else None
        for layer in self.layers:
            x = layer.forward(x) if self.dropout is None else self.dropout.forward(layer.forward(x))
...
def adam_decay_search(train_loader, validation_loader, decays, in_dim, out_dim, hidden_dim,
                      lr, betas, std, num_epochs=75, verbose=False):
    heat_map = [0 for j in decays]
    best_loss = 10000000
    for i, d in enumerate(decays):
        net = Net(in_dim, out_dim, hidden_dim, std=std)
        optimizer = torch.optim.Adam(net.params(), lr=lr, betas=betas, weight_decay=d)
        train(net, train_loader, optimizer, num_epochs=num_epochs)
        validation_loss, acc = run(net, validation_loader)
        if validation_loss < best_loss:
            best_loss = validation_loss
...
def sgd_decay_search(train_loader, validation_loader, decays, in_dim, out_dim, hidden_dim, lr, m, std, num_epochs=75,
                     verbose=False):
    heat_map = [0 for j in decays]
    best_loss = 10000000
    for i, d in enumerate(decays):
        net = Net(in_dim, out_dim, hidden_dim, std=std)
        optimizer = torch.optim.SGD(net.params(), lr=lr, momentum=m, weight_decay=d)
        train(net, train_loader, optimizer, num_epochs=num_epochs)
        validation_loss, acc = run(net, validation_loader)
        if validation_loss < best_loss:
            best_loss = validation_loss
...
def adam_dropout_search(train_loader, validation_loader, dropout_pobs, in_dim, out_dim, hidden_dim,
                        lr=0.0005, betas=(0.9, 0.999), std=0.02, num_epochs=75, verbose=False):
    heat_map = [0 for j in dropout_pobs]
    best_loss = 10000000
    for i, p in enumerate(dropout_pobs):
        net = Net(in_dim, out_dim, hidden_dim, std=std, dropout_p=p)
        optimizer = torch.optim.Adam(net.params(), lr=lr, betas=betas)
        train(net, train_loader, optimizer, num_epochs=num_epochs)
        validation_loss, acc = run(net, validation_loader)
        if validation_loss < best_loss:
...
def train(net, loader, optimizer, test_set=None, verbose=False, num_epochs=75):
    ...
    for epoch in range(num_epochs):
        ...
        if test_set is not None:
            ...
            if verbose:
                print('Epoch: {}, loss: {}, acc: {}, test loss: {}, test acc: {}'.format(epoch, train_loss, train_acc,
                                                                                         test_loss, test_acc))
        elif verbose:
            print('Epoch: {}, loss: {}, acc: {}, test loss: {}, test acc: {}'.format(epoch, train_loss, train_acc,
                                                                                     test_loss, test_acc))
